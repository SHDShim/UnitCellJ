...
# IO functions
def data2df(data, wavelength):
    df = pd.DataFrame(data, columns=['h',	'k', 'l', 'twoth'])
    df['dsp'] = wavelength / 2. / np.sin(np.deg2rad(df['twoth']/2.))
    df['Q'] = (1./df['dsp'])**2.
    return df
...
def make_output_table(results, data_df):
    """
    ...
    """
    res_lin, res_nlin = results[0], results[1]
    output = data_df.copy()
    ...
    # upon deletion of the data point as a multiple of the standard deviation for Q^2
...
def fit_cubic_cell(data_df, wavelength, verbose=True):
    ...
    cell_param_nlin = [a_nlin]
    s_cell_param_nlin = [s_a_nlin]
    results = [res_lin, res_nlin]
    
    return cell_param_nlin, s_cell_param_nlin, v_nlin, s_v_nlin, results
...
def fit_nl_cubic_cell(data_df, a, wavelength, verbose=True):
    ...
    """
    h = data_df['h']; k = data_df['k']; l = data_df['l']
    param = Parameters()
    param.add('a', value=a, min=0)
    twoth_data = data_df['twoth']
    minner = Minimizer(fcn2min_cubic, param, fcn_args=(h, k, l, twoth_data, wavelength))
...
def fit_tetragonal_cell(data_df, wavelength, verbose=True):
    ...
    a_lin_star = ufloat(res_lin.params['Prefactor0'], res_lin.bse['Prefactor0'])
    c_lin_star = ufloat(res_lin.params['Prefactor1'], res_lin.bse['Prefactor1'])
    a_lin = umath.sqrt(1./a_lin_star)
    ...
    a_res_nlin = ufloat(res_nlin.params['a'].value, res_nlin.params['a'].stderr)
    c_res_nlin = ufloat(res_nlin.params['c'].value, res_nlin.params['c'].stderr)
    ...
    if np.abs(a_lin.nominal_value - a_nlin) > a_lin.std_dev:
        print('Difference between nonlinear and linear results exeed the error bar.')
        
    cell_param_nlin = [a_nlin, c_nlin]
    s_cell_param_nlin = [s_a_nlin, s_c_nlin]
    results = [res_lin, res_nlin]
...
def fit_nl_tetragonal_cell(data_df, a, c, wavelength, verbose=True):
    ...
    twoth_data = data_df['twoth']
    minner = Minimizer(fcn2min_tetragonal, param, fcn_args=(h, k, l, twoth_data, wavelength))
    result = minner.minimize()
    # calculate final result
...
def fcn2min_hexagonal(pars, h, k, l, twoth_data, wavelength):
    ...
    wavelength = this can be replaced with .get_base_ptn_wavelength()
    """
    hk_sq = 4./3. * (h * h + h* k + k * k); l_sq = l * l
    param = pars.valuesdict()
    a = param['a']; c = param['c']
    inv_dsp_sq = hk_sq / (a * a) + l_sq / (c * c)
    model = cal_twoth(np.sqrt(1. / inv_dsp_sq), wavelength)
...
def fit_nl_hexagonal_cell(data_df, a, c, wavelength, verbose=True):
    ...
    param.add('c', value=c, min=0)
    twoth_data = data_df['twoth']
    minner = Minimizer(fcn2min_hexagonal, param, fcn_args=(h, k, l, twoth_data, wavelength))
    result = minner.minimize()
    # calculate final result
...
def fit_orthorhombic_cell(data_df, wavelength, verbose=True):
    ...
    a_lin_star = ufloat(res_lin.params['Prefactor0'], res_lin.bse['Prefactor0'])
    b_lin_star = ufloat(res_lin.params['Prefactor1'], res_lin.bse['Prefactor1'])
    c_lin_star = ufloat(res_lin.params['Prefactor2'], res_lin.bse['Prefactor2'])
    a_lin = umath.sqrt(1./a_lin_star)
    ...
    a_res_nlin = ufloat(res_nlin.params['a'].value, res_nlin.params['a'].stderr)
    b_res_nlin = ufloat(res_nlin.params['b'].value, res_nlin.params['b'].stderr)
    c_res_nlin = ufloat(res_nlin.params['c'].value, res_nlin.params['c'].stderr)
    ...
    cell_param_nlin = [a_nlin, b_nlin, c_nlin]
    s_cell_param_nlin = [s_a_nlin, s_b_nlin, s_c_nlin]
    results = [res_lin, res_nlin]
...
def fit_nl_orthorhombic_cell(data_df, a, b, c, wavelength, verbose=True):
    ...
    param.add('c', value=c, min=0)
    twoth_data = data_df['twoth']
    minner = Minimizer(fcn2min_orthorhombic, param, fcn_args=(h, k, l, twoth_data, wavelength))
    result = minner.minimize()
    # calculate final result
